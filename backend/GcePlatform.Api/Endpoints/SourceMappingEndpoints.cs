...
public static class SourceMappingEndpoints
{
    public static WebApplication MapSourceMappingEndpoints(this WebApplication app)
    {
        ...
        app.MapGet("/source-mappings", async (int? accountId, DbConnectionFactory db) =>
        {
        ...
        app.MapPost("/source-mappings", async (CreateSourceMappingRequest req, DbConnectionFactory db) =>
        {
            using var conn = db.CreateConnection();
            ...
            var item = await conn.QuerySingleOrDefaultAsync<SourceMappingDto>(@"
                SELECT
                    OrgUnitSourceMapId, OrgUnitId, OrgUnitCode, OrgUnitName, OrgUnitType,
