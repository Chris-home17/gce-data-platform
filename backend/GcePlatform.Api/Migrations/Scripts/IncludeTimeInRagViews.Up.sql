-- ============================================================
-- Migration: IncludeTimeInRagViews — Up
-- Adds 'Time' to the DataType allowlist in the two RAG-status
-- views (App.vSiteSubmissionDetails and App.vKpiSubmissions) so
-- Time KPIs surface a Green/Amber/Red dot in the monitoring UI.
--
-- The CASE that gates RAG was previously hard-coded to
-- ('Numeric','Percentage','Currency') and silently returned NULL
-- for Time submissions, so the monitoring page rendered no dot.
--
-- CREATE OR ALTER VIEW is naturally idempotent.
-- ============================================================
