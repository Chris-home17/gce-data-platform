-- ============================================================
-- Migration: KpiScoringCategoryWeightTemplate — Up
-- Switches category weights from live-read to per-template snapshot:
--   * Adds CategoryWeightSnapshot to KPI.AssignmentTemplate +
--     KPI.Assignment (mirror).
--   * Backfills existing rows from current KPI.CategoryWeight.
--   * usp_UpsertKpiAssignmentTemplate INSERT branch resolves the
--     current account-level weight; UPDATE leaves it alone.
--   * usp_AssignKpi accepts a new @CategoryWeightSnapshot param.
--   * usp_MaterializeKpiAssignmentTemplates passes the snapshot
--     through.
--   * usp_CascadeAssignmentTemplateThresholds also cascades the
--     CategoryWeightSnapshot (unsubmitted assignments only).
--   * New usp_RefreshTemplateCategoryWeights for the explicit
--     "Re-apply to existing templates" admin action.
--   * vKpiSubmissionScores adds CategoryWeight column.
--   * vSiteCompositeScore drops the LEFT JOIN to KPI.CategoryWeight.
--   * vKpiAssignmentTemplates surfaces the snapshot for the admin UI.
--
-- Idempotent: ALTER TABLE ADD guarded with COL_LENGTH IS NULL;
-- CREATE OR ALTER is naturally idempotent.
-- ============================================================

IF COL_LENGTH('KPI.AssignmentTemplate', 'CategoryWeightSnapshot') IS NULL
    ALTER TABLE KPI.AssignmentTemplate ADD CategoryWeightSnapshot DECIMAL(9,4) NULL;
GO

IF COL_LENGTH('KPI.Assignment', 'CategoryWeightSnapshot') IS NULL
    ALTER TABLE KPI.Assignment ADD CategoryWeightSnapshot DECIMAL(9,4) NULL;
GO

-- ─── Backfill: snapshot current account-level weights onto templates ─
...
WHERE t.CategoryWeightSnapshot IS NULL;
GO

-- ─── Backfill: copy template snapshot onto existing assignments ─

UPDATE a
SET CategoryWeightSnapshot = COALESCE(t.CategoryWeightSnapshot,
    -- Fall back to the live account weight for assignments without a template
    (SELECT cw.Weight
     FROM KPI.CategoryWeight AS cw
     JOIN KPI.Definition     AS d ON d.KPIID = a.KPIID
     WHERE cw.AccountId = a.AccountId AND cw.Category = d.Category),
    1.0)
FROM KPI.Assignment              AS a
LEFT JOIN KPI.AssignmentTemplate AS t ON t.AssignmentTemplateID = a.AssignmentTemplateID
WHERE a.CategoryWeightSnapshot IS NULL;
GO

-- ============================================================
-- usp_UpsertKpiAssignmentTemplate
-- INSERT branch resolves current account-level weight by KPI's
-- category and writes it into CategoryWeightSnapshot. UPDATE
-- branch leaves the column untouched (locked).
-- ============================================================
...
BEGIN
    ...
    BEGIN
        -- Resolve the account-level CategoryWeight for this KPI's category at INSERT time.
    ...
    BEGIN
        ...
        SET PeriodScheduleID       = @PeriodScheduleID,
            ...
            -- CategoryWeightSnapshot deliberately NOT updated here. Use
            -- usp_RefreshTemplateCategoryWeights to opt into a re-snap.
            IsActive               = 1,
...
-- ============================================================
-- usp_AssignKpi — accept and persist @CategoryWeightSnapshot
-- ============================================================
...
BEGIN
    ...
    -- Default CategoryWeightSnapshot to the live account weight when caller didn't provide one.
    ...
    BEGIN
        ...
        SET IsRequired             = @IsRequired,
            ...
            -- CategoryWeightSnapshot deliberately NOT touched on UPDATE.
            IsActive               = 1,
            ModifiedOnUtc          = SYSUTCDATETIME(),
            ModifiedBy             = COALESCE(@ActorUPN, SESSION_USER)
...
-- ============================================================
-- usp_MaterializeKpiAssignmentTemplates — pass CategoryWeightSnapshot through
-- ============================================================
...
BEGIN
    ...
    DECLARE template_cursor CURSOR LOCAL FAST_FORWARD FOR
        SELECT
            t.AssignmentTemplateID, d.KPICode, t.PeriodScheduleID, sched.StartDate, sched.EndDate,
...
-- ============================================================
-- usp_CascadeAssignmentTemplateThresholds — also cascade CategoryWeightSnapshot
-- ============================================================
...
-- ============================================================
-- usp_RefreshTemplateCategoryWeights — explicit "Re-apply to existing templates"
-- ============================================================
...
-- via the existing cascade proc. Returns counts so the UI can confirm impact.
...
BEGIN
    ...
    -- 1. Re-snap each affected template from current account-level weight.
    UPDATE t
    SET CategoryWeightSnapshot = ISNULL(cw.Weight, 1.0),
        ModifiedOnUtc          = SYSUTCDATETIME(),
        ModifiedBy             = @Actor
    ...
    -- 2. Cascade each updated template to its unsubmitted assignments.
    ...
    WHILE @@FETCH_STATUS = 0
    BEGIN
        -- Count unsubmitted assignments before cascade so we can report impact.
...
-- ============================================================
-- vKpiSubmissionScores — add CategoryWeight column from snapshot
-- ============================================================
...
-- ============================================================
-- vSiteCompositeScore — use snapshot from vKpiSubmissionScores
-- ============================================================
...
-- ============================================================
-- vKpiAssignmentTemplates — surface CategoryWeightSnapshot
-- ============================================================
