...
import { AlertCircle, CheckCircle2, Lock, AlertTriangle, MoreHorizontal, Link2, Gauge } from 'lucide-react'
...
export function MonitoringView() {
  ...
  const stats = useMemo(() => {
    ...
    // Composite score across the visible site set: weighted average of the
    // per-site composites (each site weighted equally — the per-KPI weights
    // already collapsed into each site's CompositeScore).
    const sitesWithScore = items.filter((i) => i.compositeScore !== null && i.compositeScore !== undefined)
    const avgComposite = sitesWithScore.length > 0
      ? sitesWithScore.reduce((s, i) => s + (i.compositeScore ?? 0), 0) / sitesWithScore.length
      : null

    return { totalRequired, totalSubmitted, totalLocked, totalMissing, overallPct, siteCount: items.length, avgComposite }
  }, [filteredItems])
